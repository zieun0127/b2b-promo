...
| v1.37 | 2026-08-21 | 사용자 요청 반영(정렬 불일치 버그 수정): PromotionListPage가 `MbtiTestPage`/`ResultPage`/`MyPage`와 달리 왼쪽 정렬로 보이는 문제를 발견 — `index.css`의 `.promotion-list-page, .mypage { max-width: 960px; }` 규칙에 `margin: 0 auto`가 누락되어 있었음(다른 넓은 화면인 `.admin-stats-page`/`.admin-promotion-page`는 이미 갖고 있던 속성). 왼쪽 정렬로 통일하는 대신 앱 전체 컨벤션인 가운데 정렬로 통일(넓은 화면에서 왼쪽 정렬 시 오른쪽 여백만 남는 문제 방지). `docs/10-style.md`(v1.11, 3-3절에 "모든 화면 가운데 정렬" 규칙 명문화) 갱신. CSS 한 줄 수정으로 기존 160 tests·`tsc -b`·`oxlint` 영향 없이 그대로 통과, Playwright로 1440px에서 프로모션 목록/마이페이지 좌우 여백이 동일해짐을 확인 |
