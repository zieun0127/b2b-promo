...
- **완료 조건**
  ...
  - [x] (v1.7 추가) "전체" 및 16개 MBTI 유형 버튼으로 목록을 필터링할 수 있고, 완료된 결과가 있으면 자신의 유형이 기본 선택되며 없으면 "전체"가 기본 선택된다 (`filterByMbtiType` 순수 함수 + `PromotionListPage.test.tsx`: 기본값 분기 2건, 버튼 클릭 시 필터링, "전체" 복귀, 0건 시 안내 문구 포함. Playwright로 실제 관리자 계정(전체 기본 선택)에서 ISTJ 버튼 클릭 시 해당 유형 1건만 목록에 남음을 확인)
  - [x] (v1.8 추가) "인기 프로모션 TOP3"는 완료된 결과가 있으면 본인 MBTI 유형에 매핑된 프로모션 내에서만 집계하고(결과 없으면 전체 기준), 목록 필터 버튼을 다른 값으로 바꿔도 TOP3는 본인 유형 기준을 그대로 유지한다 (`PromotionListPage.tsx`: TOP3는 `selectedFilter`가 아닌 `latestResult`에서 직접 도출한 `ownTypeCode`로 계산. `PromotionListPage.test.tsx`에 본인 유형 스코핑+필터 변경에도 불변 케이스 추가. Playwright로 실제 ESTJ 판정 계정에서 TOP3가 ESTJ 매핑 프로모션 1건만 표시되고 "전체" 데이터 대비 축소됨을 확인, 테스트 계정 정리함)
  - **테스트**: `promotionBadges.test.ts`(뱃지·정렬·인기 TOP3·유형 필터 순수 함수), `promotionApi.test.ts`, `bookmarkApi.test.ts`, `PromotionCard.test.tsx`, `PromotionListPage.test.tsx` — 전체 130 tests 통과, 대상 파일(`promotionBadges.ts` 100%, `PromotionCard.tsx` 100%, `PromotionListPage.tsx` 95%) 모두 목표 90% 충족
