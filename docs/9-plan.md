...
- **완료 조건**
  ...
  - [x] (v1.7 추가) "전체" 및 16개 MBTI 유형 버튼으로 목록을 필터링할 수 있고, 완료된 결과가 있으면 자신의 유형이 기본 선택되며 없으면 "전체"가 기본 선택된다. "인기 프로모션 TOP3"는 필터의 영향을 받지 않는다 (`filterByMbtiType` 순수 함수 + `PromotionListPage.test.tsx`: 기본값 분기 2건, 버튼 클릭 시 필터링, "전체" 복귀, 0건 시 안내 문구, TOP3 불변 케이스 포함. Playwright로 실제 관리자 계정(전체 기본 선택)에서 ISTJ 버튼 클릭 시 해당 유형 1건만 목록에 남고 TOP3는 그대로 유지됨을 확인)
  - **테스트**: `promotionBadges.test.ts`(뱃지·정렬·인기 TOP3·유형 필터 순수 함수), `promotionApi.test.ts`, `bookmarkApi.test.ts`, `PromotionCard.test.tsx`, `PromotionListPage.test.tsx` — 전체 127 tests 통과, 대상 파일(`promotionBadges.ts` 100%, `promotionApi.ts` 93.3%, `bookmarkApi.ts` 90.9%, `PromotionCard.tsx` 100%, `PromotionListPage.tsx` 94.73%) 모두 목표 90% 충족
