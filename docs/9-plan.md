...
- **완료 조건**
  ...
  - [x] (v1.9 추가) 마감일이 지난 프로모션에 "마감" 뱃지, 마감일이 없는 프로모션에 "상시" 뱃지, "인기 프로모션 TOP3"에 포함된 프로모션은 목록에서도 "인기" 뱃지가 표시된다("마감임박"/"마감"/"상시"는 서로 배타적) (`isEnded`/`isAlwaysOpen` 순수 함수 추가, `PromotionListPage.tsx`가 TOP3와 동일한 id 집합을 `isPopular` prop으로 전달. `PromotionCard.test.tsx`/`PromotionListPage.test.tsx`에 케이스 추가. Playwright로 실제 DB에서 마감일을 과거로 임시 변경해 "마감" 뱃지 렌더를 확인 후 원복함)
  - **테스트**: `promotionBadges.test.ts`(뱃지·정렬·인기 TOP3·상태 필터 순수 함수), `promotionApi.test.ts`, `bookmarkApi.test.ts`, `PromotionCard.test.tsx`, `PromotionListPage.test.tsx` — 전체 146 tests 통과, 대상 파일(`promotionBadges.ts` 100%, `PromotionCard.tsx` 100%, `PromotionListPage.tsx` 95.23%) 모두 목표 90% 충족
